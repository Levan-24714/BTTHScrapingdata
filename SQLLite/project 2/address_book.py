...
def them():
    ...
    c = conn.cursor()
    # Lấy dữ liệu đã nhập
    name_value = f_name.get().lower()
    lastName_value = l_name.get().lower()
    address_value = address.get().lower()
    city_value = city.get().lower()
    state_value = state.get().lower()
    zipcode_value = zipcode.get()

    if not name_value or not lastName_value or not address_value:
        messagebox.showwarning("Thông báo", "Vui lòng nhập đầy đủ thông tin")
        return
    #kiểm tra xem thông tin đã tồn tại chưa
    c.execute('''SELECT * FROM  addresses Where LOWER(first_name) =:first_name 
    ...
    #Hàm fetchall() được gọi để lấy tất cả các bản ghi mà truy vấn tìm thấy
    result = c.fetchall()
    if result:
        #nếu đã tồn tại , hiện thông báo
        messagebox.showwarning("Thông báo", "Thông tin này đã tồn tại")
    else:
...
def cap_nhap():
    ...
    c.execute("""UPDATE addresses SET
               ...
               WHERE id = :id""",
              {
                  ...
                  'id': record_id # sử dụng ID để tìm bản ghi
              })
